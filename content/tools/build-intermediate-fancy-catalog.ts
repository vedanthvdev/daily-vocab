#!/usr/bin/env tsx
/**
 * Rebuilds intermediate.json as fancy-but-usable enrichment words:
 * precise vocabulary people know English might not use day to day,
 * below hard's rare literary tier.
 *
 * Usage: npm install --no-save wordpos && npx tsx content/tools/build-intermediate-fancy-catalog.ts
 */
...
async function main(): Promise<void> {
  ...
  const pool = JSON.parse(
    readFileSync(resolve(__dirname, '../sources/intermediate-fancy-pool.json'), 'utf8'),
  ) as PoolEntry[];
  ...
  const hard = new Set(
    (
      JSON.parse(readFileSync(resolve(__dirname, '../words/hard.json'), 'utf8')) as {
        words: { word: string }[];
      }
    ).words.map((w) => w.word),
  );
  ...
  for (const entry of pool) {
    ...
    if (!/^[a-z][a-z'-]*$/.test(word) || word.length < 4) continue;
    if (seen.has(word) || beginner.has(word) || hard.has(word) || ESL_DENYLIST.has(word)) {
      continue;
    }
    ...
    seen.add(word);
    words.push({
      id: `i${String(words.length + 1).padStart(3, '0')}`,
      word,
      oneLiner,
    });
  }
  ...
  if (words.length < STRICT_WORD_COUNT) {
    throw new Error(`Only produced ${words.length} intermediate words; need ${STRICT_WORD_COUNT}`);
  }

  const outPath = resolve(__dirname, '../words/intermediate.json');
  writeFileSync(
    outPath,
    JSON.stringify({ version: 1, level: 'intermediate', words }, null, 2) + '\n',
  );
  console.log(`wrote ${outPath} (${words.length} fancy words)`);
